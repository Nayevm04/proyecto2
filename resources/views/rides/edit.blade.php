...
<div class="max-w-3xl mx-auto bg-white shadow-md p-6 rounded-lg">
    ...
    <h2 class="text-2xl font-bold mb-4">Editar Ride</h2>
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div>
                ...
                <input type="text" name="nombre" class="w-full border rounded p-2"
                       value="{{ old('nombre', $ride->nombre) }}" required>
            </div>

            <div>
                <label class="font-semibold">Lugar de salida</label>
                <input type="text" name="lugar_salida" class="w-full border rounded p-2"
                       value="{{ old('lugar_salida', $ride->lugar_salida) }}" required>
            </div>

            <div>
                <label class="font-semibold">Lugar de llegada</label>
                <input type="text" name="lugar_llegada" class="w-full border rounded p-2"
                       value="{{ old('lugar_llegada', $ride->lugar_llegada) }}" required>
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="fecha_hora"
                       ...
                       value="{{ old('fecha_hora', $ride->fecha . 'T' . $ride->hora) }}" required>
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="costo_espacio"
                       class="w-full border rounded p-2"
                       value="{{ old('costo_espacio', $ride->costo) }}" required>
            </div>
            ...
            <div>
                ...
                <input type="number" name="cantidad_espacios"
                       class="w-full border rounded p-2"
                       value="{{ old('cantidad_espacios', $ride->cantidad_espacios) }}" required>
            </div>
                ...
                <select name="vehiculo_id" class="w-full border rounded p-2" required>
                    @foreach ($vehiculos as $vehiculo)
                        <option value="{{ $vehiculo->id }}"
                            {{ old('vehiculo_id', $ride->vehiculo_id) == $vehiculo->id ? 'selected' : '' }}>
